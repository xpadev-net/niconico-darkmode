...
window.onload = function () {
  ...
  //総合TOP ボタン
  let nicodark_setting_cb_s_top = document.getElementById(
    "nicodark_s_top-setting_cb"
  );
  let nicodark_aria_s_top = document.getElementById("nicodark-aria-s_top");

  //OS設定と合わせる ボタン
  let nicodark_set_os_settings_cb = document.getElementById(
    "nicodark_set_os_settings_cb"
  );
  let nicodark_aria_set_os_settings = document.getElementById(
    "nicodark-aria-set_os_settings"
  );

  //バージョン表示
  document.getElementById("show_version").textContent =
    "nicodark v" + EX_VERSION;
  ...
  nicodark_setting_cb.onclick = function () {
    if (is_darkmode === "false") {
      chrome.runtime.sendMessage(
        { request_change_settings: "req_nicodark_to_true" },
        function (response) {
      ...
      chrome.runtime.sendMessage(
        { request_change_settings: "req_nicodark_to_false" },
        function (response) {
          console.log(response.farewell);
        }
      );
  ...
  nicodark_setting_cb_s_top.onclick = function () {
    if (is_socialtop === "false") {
      chrome.runtime.sendMessage(
        { request_change_settings: "req_nicodark_top_to_true" },
        function (response) {
          console.log(response.farewell);
        }
      );
      ...
      chrome.runtime.sendMessage(
        { request_change_settings: "req_nicodark_top_to_false" },
        function (response) {
          console.log(response.farewell);
        }
      );
  ...
  nicodark_set_os_settings_cb.onclick = function () {
    if (isset_osdarkmode === "false") {
      chrome.runtime.sendMessage(
        { request_change_settings: "req_nicodark_osset_to_true" },
        function (response) {
          console.log(response.farewell);
        }
      );
      ...
      chrome.runtime.sendMessage(
        { request_change_settings: "req_nicodark_osset_to_false" },
        function (response) {
          console.log(response.farewell);
        }
      );
